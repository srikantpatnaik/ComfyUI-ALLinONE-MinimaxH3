...
- **Output favorites and actions** — toggle the output strip between all and favorite media, star the selected preview beside its resolution, and use the right-click menu to copy a video into ComfyUI input or send it to Extend.
- **Library management** — favorite outputs show a visible star, Library supports per-item selection and bulk deletion (selected, non-favorites, or all), and favorites can be downloaded as a ZIP archive.
...
- **Upstream single-pass sampling** — normal T2V and I2V renders use the original H3 workflow and one native sampling pass for the requested duration. Automatic temporal chunking is disabled, so longer or larger renders use more VRAM instead of repeating sampling chunks.
- **Native best-quality sampling** — the Tune panel no longer exposes Temporal batches or Quality presets. It offers independent SolAttn, H3 Cache, and SageAttn selections; disabled selections leave the native H3 path untouched. Steps remain under user control, and Turbo/low-VRAM attention patches are not inserted.
...
- **LTX-2.5 I2V tab** — the same node now includes a separate `LTX 2.5` tab beside the default `H3` tab. It builds the local two-pass LTX-2.5 image-to-video workflow, supports Original/16:9/9:16 aspect selection, native resolution, duration, FPS, optional audio, EasyCache, and post-sampling RIFE, and submits exactly one final video. Its multi-LoRA editor supports up to eight LoRAs with individual strengths, directly on the LTX page as well as in Settings; its text encoder, diffusion model, video/audio VAEs, and latent upscaler remain selectable in Settings in the same style as H3. H3 and LTX jobs can be switched and queued from the same node; output thumbnails label the provider as `H3` or `LTX2.5`.
