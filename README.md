...
- **LTX-2.5 I2V tab** — the same node now includes a separate `LTX 2.5` tab beside the default `H3` tab. It builds the local two-pass LTX-2.5 image-to-video workflow, supports Original/16:9/9:16 aspect selection, native resolution, duration, FPS, optional audio, EasyCache, and post-sampling RIFE, and submits exactly one final video. Its compatible LoRA is selectable directly on the LTX page as well as in Settings; its text encoder, diffusion model, video/audio VAEs, and latent upscaler remain selectable in Settings in the same style as H3. H3 and LTX jobs can be switched and queued from the same node; output thumbnails label the provider as `H3` or `LTX2.5`.
